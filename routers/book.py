...
@router.post('/book')
def create(request: schemas.BookCreate, db: Session = Depends(get_db), current_user: schemas.UserBase = Depends(get_current_user)):
...
@router.put('/book/{id}')
def update(id: int, request: schemas.BookCreate, db: Session = Depends(get_db), current_user: schemas.UserBase = Depends(get_current_user)):
